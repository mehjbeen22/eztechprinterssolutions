const PrinterOffline = () => {
  return (
    <main className="flex flex-col justify-center items-center p-5 bg-white min-h-screen">
      <h1 className="text-4xl font-bold mb-8 text-center text-[#115e59]">
        Printer Offline Setup
      </h1>
      <section className="shadow-lg w-[95%] p-10 bg-gray-50 rounded-lg">
        {printerSetupContent.map((item) => (
          <div key={item.id} className="my-8">
            <h2 className="text-2xl font-bold mb-4 text-[#115e59]">
              {item.head}
            </h2>
            {item.description && (
              <p className="text-lg mb-4 text-gray-700">{item.description}</p>
            )}
